...
# Configure rpy2 for SAS Model Manager Service
...
The Model Manager service also includes support for testing and deploying R
...
1. Copy the files in the `$deploy/sas-bases/examples/sas-model-manager/r`
   directory to the `$deploy/site-config/sas-model-manager/r` directory.
...
4. Add site-config/sas-model-manager/r/rpy2-transformer.yaml to the
   transformers block to the base kustomization.yaml file in the `$deploy`
   directory.

   ```yaml
   transformers:
     - site-config/sas-model-manager/r/rpy2-transformer.yaml
   ```
